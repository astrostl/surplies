...
func TestActivePayloadHashMetadata(t *testing.T) {
	...
	for _, h := range KnownRepoPayloadHashes {
		if len(h.SHA256) != 64 || !validGitObjectID(h.SHA256) || h.Size <= 0 || h.Size >= SignatureScanMaxBytes || seen[h.SHA256] {
...
func TestGitMissingObjectStillScansAvailableBlobs(t *testing.T) {
	...
	if s.stats.GitBlobsConsidered != 1 || s.stats.GitBlobsChecked != 1 || s.stats.GitRepositoriesScanned != 0 {
