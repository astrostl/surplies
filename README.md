...
Scheduled scans use the default options plus `-q`. Only critical findings (exit code 2) raise a desktop notification; clean scans, warning-level findings, and incomplete coverage are silent. Run `surplies` yourself for the details. Linux additionally requires a running systemd user manager, `notify-send` (libnotify), and a desktop notification session; the prerequisites are checked before anything is written.
