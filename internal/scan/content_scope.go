...
// entry may be nil when the caller has no directory entry in hand. The size
// check is deliberately last: it costs a stat, so it runs only for files every
// cheaper rule already rejected.
func (s *Scanner) routineContentFile(path, name string, entry os.DirEntry) bool {
	...
	if s.injectionContentFile(path, name) {
		return true
	}
	return hashSizeCandidate(entry)
}

// hashSizeCandidate admits an unrecognized filename whose length exactly
// matches a published payload. This is what makes a renamed dropper reachable
// without reading every file on disk.
func hashSizeCandidate(entry os.DirEntry) bool {
