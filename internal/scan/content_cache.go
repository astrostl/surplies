...
func (s *Scanner) readCachedContent(f *os.File, path string, source bool, stats *contentReadStats) ([]byte, bool, error) {
	...
	// Exact-hash candidates keep the full-read policy whether they were
	// selected by published filename or by exact size under another name.
	candidate := knownPayloadName(filepath.Base(path)) || before != nil && knownPayloadSize(before.Size())
	data, binary, err := readScanContent(f, strings.ToLower(filepath.Ext(path)), source && !candidate, stats, candidate)
	after, _ := f.Stat()
