...
An abandoned subtree is a CRITICAL coverage verdict, not a warning, for the same reason as the Git record below: the scan stopped covering part of the machine. Everything past the third timeout is unread, and a payload sitting in a skipped directory is reported by nothing — so a run that gave up on a synced folder full of repositories must not print `no critical indicators` unqualified. Like every coverage verdict it is never counted as an attack indicator; the summary says coverage failed critically, and the run exits 2. The finding also names the number of files skipped after the threshold, because one record standing for an unbounded remainder gives a reader no way to weigh what was lost.

Coverage limitations appear in a compact, separate summary in text output; they are not counted as attack indicators. The summary counts size-limit failures, permission denials, timeouts, Git errors, Git coverage, network collection errors, and other errors separately. Coverage failures list paths grouped by category and shared cause: each explanation appears once, followed by sorted affected paths. Routine scope notices are summarized by category, with exact paths retained in JSON. JSON retains each exact error or read limit. JSON retains individual `scan-incomplete` records, and incomplete coverage still produces a nonzero exit status and an explicit qualification in the final result.
